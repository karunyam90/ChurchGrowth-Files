...
class Ithemes_Updater_Settings_Page {
	...
	private function save_settings() {
		...
		$this->messages[] = __( 'Settings saved', 'rcp-custom-redirects' );
	}
	...
	private function license_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username and password in order to license products.', 'rcp-custom-redirects' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username in order to license products.', 'rcp-custom-redirects' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership password in order to license products.', 'rcp-custom-redirects' );
		else if ( empty( $data['packages'] ) )
			$this->errors[] = __( 'You must select at least one product to license. Ensure that you select the products that you wish to license in the listing below.', 'rcp-custom-redirects' );
		...
		if ( empty( $response['packages'] ) ) {
			$this->errors[] = __( 'An unknown server error occurred. Please try to license your products again at another time.', 'rcp-custom-redirects' );
			return;
		}
		...
		foreach ( $response['packages'] as $package => $data ) {
			...
			if ( ! empty( $data['key'] ) )
				...
				$warn[$name] = __( 'Your product subscription has expired', 'rcp-custom-redirects' );
			else
		...
		if ( ! empty( $success ) )
			$this->messages[] = wp_sprintf( __( 'Successfully licensed %l.', 'rcp-custom-redirects' ), $success );

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason )
				$this->errors[] = sprintf( __( 'Unable to license %1$s. Reason: %2$s', 'rcp-custom-redirects' ), $name, $reason );
		}

		if ( ! empty( $warn ) ) {
			foreach ( $warn as $name => $reason )
				$this->soft_errors[] = sprintf( __( 'Unable to license %1$s. Reason: %2$s', 'rcp-custom-redirects' ), $name, $reason );
		}
	}
	...
	private function unlicense_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username and password in order to remove licenses.', 'rcp-custom-redirects' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username in order to remove licenses.', 'rcp-custom-redirects' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership password in order to remove licenses.', 'rcp-custom-redirects' );
		else if ( empty( $data['packages'] ) )
			$this->errors[] = __( 'You must select at least one license to remove. Ensure that you select the licenses that you wish to remove in the listing below.', 'rcp-custom-redirects' );
		...
		if ( empty( $response['packages'] ) ) {
			$this->errors[] = __( 'An unknown server error occurred. Please try to remove licenses from your products again at another time.', 'rcp-custom-redirects' );
			return;
		}
		...
		foreach ( $response['packages'] as $package => $data ) {
			...
			if ( isset( $data['status'] ) && ( 'inactive' == $data['status'] ) )
				...
				$fail[$name] = __( 'Unknown server error.', 'rcp-custom-redirects' );
		}


		if ( ! empty( $success ) )
			$this->messages[] = wp_sprintf( _n( 'Successfully removed license from %l.', 'Successfully removed licenses from %l.', count( $success ), 'rcp-custom-redirects' ), $success );

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason )
				$this->errors[] = sprintf( __( 'Unable to remove license from %1$s. Reason: %2$s', 'rcp-custom-redirects' ), $name, $reason );
		}
	}
	...
	public function list_packages( $action = 'list_packages', $post_data = array() ) {
		...
		if ( ! empty( $_REQUEST['updated_url'] ) ) {
			$this->messages[] = __( 'Successfully updated the Licensed URL.', 'rcp-custom-redirects' );
		}
...
?>
	<div class="wrap">
		<h2><?php _e( 'iThemes Licensing', 'rcp-custom-redirects' ); ?></h2>
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>">
		...
		<div id="ithemes-updater-settings">
			<h3 class="subtitle"><?php _e( 'Settings', 'rcp-custom-redirects' ); ?></h3>

			<table class="form-table">
				<tbody>
					<tr valign="top">
						<th scope="row">
							<?php _e( 'Licensed URL', 'rcp-custom-redirects' ); ?>
						</th>
						<td>
							<p>
								<code><?php echo $GLOBALS['ithemes-updater-settings']->get_licensed_site_url(); ?></code>
								<a href="<?php echo admin_url( 'options-general.php?page=ithemes-licensing&action=change_licensed_site_url' ); ?>" class="button button-primary"><?php _e( 'Change', 'rcp-custom-redirects' ); ?></a>
							</p>

							<?php if ( is_multisite() ) : ?>
								<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress network. If this is not set correctly, some features may not function as expected.', 'rcp-custom-redirects' ); ?></p>
							<?php else : ?>
								<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress site. If this is not set correctly, some features may not function as expected.', 'rcp-custom-redirects' ); ?></p>
							<?php endif; ?>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row">
							<label for="quick_releases"><?php _e( 'Quick Release Updates', 'rcp-custom-redirects' ); ?></label>
						</th>
						<td>
			...
			<p class="submit">
				<input id="save_settings" class="button button-primary" type="submit" value="<?php _e( 'Save Settings', 'rcp-custom-redirects' ); ?>" />
...
<?php
	...
	private function list_licensed_products( $products, $post_data, $action ) {
		...
		$headings = array(
			__( 'Product', 'rcp-custom-redirects' ),
			__( 'Member', 'rcp-custom-redirects' ),
			__( 'Expiration', 'rcp-custom-redirects' ),
			__( 'Remaining Licenses', 'rcp-custom-redirects' ),
		);
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-licensed">
			<h3 class="subtitle"><?php _e( 'Licensed Products', 'rcp-custom-redirects' ); ?></h3>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-1"><?php _e( 'Product', 'rcp-custom-redirects' ); ?></label>
						</th>
						<th scope="col"><?php _e( 'Member', 'rcp-custom-redirects' ); ?></th>
						<th scope="col"><?php _e( 'Product Status', 'rcp-custom-redirects' ); ?></th>
						<th scope="col"><?php _e( 'Expiration', 'rcp-custom-redirects' ); ?></th>
						<th scope="col"><?php _e( 'Remaining Licenses', 'rcp-custom-redirects' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php $count = 0; ?>
					<?php foreach ( $products as $name => $data ) : ?>
						<?php
							if ( -1 == $data['total'] )
								$remaining = __( 'unlimited', 'rcp-custom-redirects' );
							else
								$remaining = $data['total'] - $data['used'];

//							if ( 0 == $remaining )
//								$remaining .= ' <a class="button-secondary upgrade">' . __( 'Upgrade', 'rcp-custom-redirects' ) . '</a>';
				...
				<tfoot>
					<tr>
						<td colspan="6">
							...
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'Remove Licenses', 'rcp-custom-redirects' ); ?>" />
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-unlicensed">
			<h3 class="subtitle"><?php _e( 'Unlicensed Products', 'rcp-custom-redirects' ); ?></h3>

			<p><?php _e( 'The following products have not been licensed. Licensing a product gives you access to automatic updates from within WordPress.', 'rcp-custom-redirects' ); ?></p>
			<p><?php _e( 'To license products, select the products you wish to license, enter your iThemes membership username and password, and press the License Products button.', 'rcp-custom-redirects' ); ?></p>
			<p><?php printf( __( 'Need help? <a href="%s">Click here for a quick video tutorial</a>.', 'rcp-custom-redirects' ), 'http://ithemes.com/2013/04/11/introducing-the-new-and-improved-ithemes-licensing-system/' ); ?></p>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-2"><?php _e( 'Product', 'rcp-custom-redirects' ); ?></label>
						</th>
					</tr>
				</thead>
				<tbody>
				...
				<tfoot>
					<tr>
						<td colspan="2">
							<input type="text" name="it-updater-username" placeholder="iThemes Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'License Products', 'rcp-custom-redirects' ); ?>" />
...
?>
	<div class="ithemes-updater-products" id="ithemes-updater-unrecognized">
		<h3 class="subtitle"><?php _e( 'Unrecognized Products', 'rcp-custom-redirects' ); ?></h3>

		<p><?php _e( 'The following products were not recognized by the licensing system. This can be due to a bug in the product code, a temporary server issue, or because the product is no longer supported.', 'rcp-custom-redirects' ); ?></p>
		<p><?php printf( __( 'Please check this page again at a later time to see if the problem resolves itself. If the product remains, please contact <a href="%s">iThemes support</a> and provide them with the details given below.', 'rcp-custom-redirects' ), 'http://ithemes.com/support/' ); ?></p>

		<table class="ithemes-updater-listing widefat">
			<thead>
				<tr>
					<th scope="col"><?php _e( 'Product', 'rcp-custom-redirects' ); ?></th>
					<th scope="col"><?php _e( 'Type', 'rcp-custom-redirects' ); ?></th>
					<th scope="col"><?php _e( 'Package', 'rcp-custom-redirects' ); ?></th>
					<th scope="col"><?php _e( 'Version', 'rcp-custom-redirects' ); ?></th>
					<th scope="col"><?php _e( 'Server Response', 'rcp-custom-redirects' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php $count = 0; ?>
				<?php foreach ( $products as $name => $data ) : ?>
					<?php
						if ( ( isset( $data['status'] ) && 'error' == $data['status'] ) && ( ! empty( $data['error']['message'] ) ) )
							...
							$response = __( 'Unknown Error', 'rcp-custom-redirects' );
...
?>
	<div class="wrap" id="ithemes-updater-site-url-confirmation">
		...
		<h2><?php _e( 'Licensing', 'rcp-custom-redirects' ); ?></h2>

		<p><?php _e( "Please confirm this site's licensed URL.", 'rcp-custom-redirects' ); ?></p>
		...
		<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>">
			...
			<div id="ithemes-updater-settings">
				<div id="ithemes-updater-table-wrapper">
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
									<label for="site_url"><?php _e( 'Licensed URL', 'rcp-custom-redirects' ); ?></label>

									<?php if ( is_multisite() ) : ?>
										<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress network.', 'rcp-custom-redirects' ); ?></p>
										<p class="description ithemes-updater-description-warning"><?php _e( 'If not set correctly, some features may not function as expected.', 'rcp-custom-redirects' ); ?></p>
									<?php else : ?>
										<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress site.', 'rcp-custom-redirects' ); ?></p>
										<p class="description ithemes-updater-description-warning"><?php _e( 'If not set correctly, some features may not function as expected.', 'rcp-custom-redirects' ); ?></p>
									<?php endif; ?>
								</th>
								<td>
				...
				<p class="submit">
					<input id="save_licensed_site_url" class="button button-primary" type="submit" value="<?php _e( 'Save', 'rcp-custom-redirects' ); ?>" />
...
<?php
	...
	private function save_licensed_site_url( $data ) {
		...
		if ( empty( $data['site_url'] ) ) {
			$this->errors[] = __( 'The licensed URL cannot be blank.', 'rcp-custom-redirects' );
		} else if ( false === filter_var( $data['site_url'], FILTER_VALIDATE_URL ) ) {
			$this->errors[] = __( 'The licensed URL must be a valid URL.', 'rcp-custom-redirects' );
		}
		...
		$this->messages[] = __( 'Successfully set the Licensed URL.', 'rcp-custom-redirects' );
...
?>
	<div class="wrap" id="ithemes-updater-relicense">
		...
		<p><?php printf( __( 'The licenses on this site are for <code>%s</code>.', 'rcp-custom-redirects' ), $site_url_from_server ); ?></p>
		...
		<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>">
			...
			<div id="ithemes-updater-settings">
				<div id="ithemes-updater-table-wrapper">
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
									<label for="relicense_option">
										<?php _e( 'License Option', 'rcp-custom-redirects' ); ?>
									</label>
								</th>
								<td>
									<p>
										<label>
											...
											<?php _e( 'Create new licenses for this site.', 'rcp-custom-redirects' ); ?>
										</label>
									</p>
									<p class="description ithemes-updater-description-warning"><?php _e( 'Use this option if this site was cloned from another site and needs to have its own licenses.', 'rcp-custom-redirects' ); ?></p>
									<br />

									<p>
										<label>
											<input type="radio" name="relicense_option" value="update" <?php if ( 'update' === $data['relicense_option'] ) echo 'checked="checked"'; ?> />
											<?php printf( __( 'Change the existing licenses to be for <code>%s</code>.', 'rcp-custom-redirects' ), $data['site_url'] ); ?>
										</label>
									</p>
									<p class="description ithemes-updater-description-notice">
										<span class="dashicons dashicons-warning"></span>
										<?php printf( __( 'Note: If the <code>%s</code> site still exists and is different from this site, you will have to create new licenses on that site.', 'rcp-custom-redirects' ), $data['site_url'], $site_url_from_server ); ?>
									</p>
									<p class="description ithemes-updater-description-warning"><?php printf( __( 'Use this option if this site\'s primary URL has changed from <code>%1$s</code> to <code>%2$s</code>.', 'rcp-custom-redirects' ), $site_url_from_server, $data['site_url'] ); ?></p>
								</td>
							</tr>
							<tr valign="top">
								<th scope="row" colspan="2">
									<label for="it-updater-username"><?php _e( 'iThemes Username', 'rcp-custom-redirects' ); ?></label>
									<br />
							...
							<tr valign="top">
								<th scope="row" colspan="2">
									<label for="it-updater-password"><?php _e( 'iThemes Password', 'rcp-custom-redirects' ); ?></label>
									<br />
				...
				<p class="submit">
					<input id="relicense" class="button button-primary" type="submit" value="<?php _e( 'Save', 'rcp-custom-redirects' ); ?>" />
...
<?php
	...
	private function relicense( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply an iThemes membership username and password in order to change the licensed URL.', 'rcp-custom-redirects' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply an iThemes membership username in order to change the licensed URL.', 'rcp-custom-redirects' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply an iThemes membership password in order to change the licensed URL.', 'rcp-custom-redirects' );
		} else if ( empty( $data['site_url'] ) ) {
			$this->errors[] = __( 'The licensed URL cannot be blank.', 'rcp-custom-redirects' );
		} else if ( false === filter_var( $data['site_url'], FILTER_VALIDATE_URL ) ) {
			$this->errors[] = __( 'The licensed URL must be a valid URL.', 'rcp-custom-redirects' );
		} else if ( empty( $data['relicense_option'] ) || ! in_array( $data['relicense_option'], array( 'relicense', 'update' ) ) ) {
			$this->errors[] = __( 'You must pick one of the License Option options.', 'rcp-custom-redirects' );
		}
	...
	private function get_expiration_string( $expiration_timestamp ) {
		...
		if ( $time_left > ( 86400 * 30 ) )
			...
			if ( $time_left > 86400 )
				$expiration = sprintf( _n( '%d day', '%d days', intval( $time_left / 86400 ), 'rcp-custom-redirects' ), intval( $time_left / 86400 ) );
			else if ( $time_left > 3600 )
				$expiration = sprintf( _n( '%d hour', '%d hours', intval( $time_left / 3600 ), 'rcp-custom-redirects' ), intval( $time_left / 3600 ) );
			else if ( $time_left > 60 )
				$expiration = sprintf( _n( '%d minute', '%d minutes', intval( $time_left / 60 ), 'rcp-custom-redirects' ), intval( $time_left / 60 ) );
			else
				$expiration = sprintf( _n( '%d second', '%d seconds', $time_left, 'rcp-custom-redirects' ), intval( $time_left / 60 ) );

			if ( $expired )
				$expiration = sprintf( __( '%s ago', 'rcp-custom-redirects' ), $expiration );
		}
